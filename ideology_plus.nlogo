...
agents-own[
  ...
  ; history_disagree_others    ; a n-agents * n-back matrix (list?) that tracks each agents' history of interactions - i.e., the % of the previous n-back ticks that they disagreed
                             ; one row for each agent, including self (set to 0), 5 cols for previous interactions - MOVED TO LINK VAR history_interactions
...
influences-own [
  ...
  history_interactions                        ; and have a memory of interactions between agents at either end
]
...
to setup-world
  ...
  set sigmoid_s 6                                             ; sets the slope of sigmoid function
                                                              ; note - sigmoid function used here has been changed from that in the original paper
  set outgoing_info_M matrix:make-constant n-agents 5 0       ; empty n-agents * 5 matrix, ready to store

  ;
  ; set n-back 5        ; agents currently only consider the past 5 interactions with agents
...
to setup-agents
 ...
 create-agents (n-agents) [
    ...
    ; new stuff (tolerance)
    set weight_slope .01 + random-normal sigmoid_s 3        ; draw slopes for sigmoid from normal distribution with mean determined by sigmoid_s and sd of 1
    ; set weight_slope random-float 6                       ; OR from uniform 0 - 6

    ;set history_disagree_others matrix:make-constant n-agents n-back 0       ; set up empty matrixm, each agent has row and 5 time points to fill
    ; print matrix:pretty-print-text history_disagree_others
      ...
      foreach list_agents [
        ...
        if (i != j) and (matrix:get influence_m i j != 0)[           ; only run if i and j are difference agents AND they share a tie at present
            ...
            ifelse (cosim_ij >= 0) [                                 ; when agents' opinions align
              ...
              matrix:set influence_m i j sigmoid slope .5 cosim_ij      ; if which case, ties strengthened when agents' opinions align via the sigmoid function
            ] [                                                    ; OR
              let slope sigmoid_s                                    ; the value of the global var sigmoid_s
              matrix:set influence_m i j sigmoid slope .5 cosim_ij      ; again, ties strengthened when agents' opinons align via the sigmoid function
            ]
