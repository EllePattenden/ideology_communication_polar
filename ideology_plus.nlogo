...
globals [
  ...
  ; new additions
  ;n-back           ; how many previous interactions (ticks) agents consider when deciding how to update ties (i.e., what the threshold % set on the interface is applying to)
                    ; now set on interface as well
  components        ; track how many components are present
  cosim_components       ; cosine similarity of (first 2) components that emerge in simulations
  corr_status_stubb      ; (pearson) correlation between agent status and stubborness - not particuarly useful
]
...
agents-own[
  ...
  ; social (Elle)
  type_social       ; determines how agents' weight information from others based on the cosine similarity of their opinions (changing two parameters in the sigmoid function)
                    ; "previous" = sigmoid function used in Kashima et al. (2021) - "weak_sim_bias" = considers views of anyone with cosine similarity > 0, with weak association between cosine_sim and weight
                    ; "strong_sim_bias"   =  considers views of anyone with cosine similarity > 0, but weights similar others more highly
                    ; "extreme_sim_bias"  =  only weights opinions of very similar others
                    ; "very_weak_sim_bias" = removes constraint that cosine_similarity needs to be > 0 for agents opinion to be considered.


  ; new additions - changing how agents' update the weight of their influence ties
  weight_slope               ; individual differences variable determining the steepness of the signmoid function;
                             ; higher values = stronger relationship between cosine similarity and link weight
  weight_shift               ; individual difference variable determining the how far the sigmoid function is shifted to the right
...
influences-own [
  weight                                      ; links store their weight
  history_interactions                        ; tracks each agents' history of n-back interactions (i.e., have memory)
]
...
to setup
  ...
  setup-world
  setup-agents
  ;print word "average slope: " mean [weight_slope] of agents
  friedkin_as_matrix         ; matrices made life much easier here

  ; compute correlation between agent status and stubborness
  let data [(list status stubborness)] of agents
  let m matrix:from-row-list data
  let m-reg matrix:regress m
  set corr_status_stubb item 0 item 1 m-reg
  print word "correlation between status and stubborness: " item 0 item 1 m-reg
end
...
to setup-agents
 ...
 create-agents (n-agents) [
    ...
    set self_rep fill-matrix 1 5 [-> (-1 + (random-float 2 ) )]
    ...
    ; Social traits (Elle)
;    set type_social social_types                       ; set on interface
;    if type_social = "previous" [
;    ]
;    if type_social = "strong_sim_bias" [
;    ]
;    if type_social = "extreme_sim_bias" [
;    ]
;    if type_social = "very_weak_sim_bias" [
;    ]


    ; new stuff (tolerance)
    set weight_slope .01 + random-normal sigmoid_s 2        ; draw slopes for sigmoid from normal distribution with mean determined by sigmoid_s and sd of 4
...
to friedkin_as_matrix
  ...
  if  starting_network = "Erdős–Rényi" [
    random_starting
  ]

  if starting_network = "fully_connected" [
    fully_connected_starting
  ]
  ...
  let list_agents2 range n-agents
  foreach list_agents2 [
    i ->
    foreach list_agents2 [
      j ->
      if i != j [
        let weight_ij matrix:get influence_m i j
        ask an-agent i [
          create-influence-to an-agent j [
            set weight weight_ij                     ; store weight from matrix as link var
            set history_interactions n-values n-back [0]      ; create  list to store history of the past n-back interactions between
          ]
        ]
      ]
    ]
...
to fully_connected_starting               ; intialise a network where every agent agent shares a (directed) tie but with different weights
  let list_agents range n-agents                                       ; create a list of agents to loop through (order important)
  foreach list_agents [
    i ->
    foreach list_agents [
     j ->
      let stubborn_i [stubborness] of an-agent i
      let  status_i [status] of an-agent i
      if i != j [
        let stubborn_j [stubborness] of an-agent j
        let status_j [status] of an-agent j
        matrix:set influence_m i j sigmoid sigmoid_s .5 (status_i - status_j)       ; the social influence i feels from j depends on how much higher j's status is than i's
...
to go
if ticks = 200 [stop]        ; CHANGE SO MODEL RUNS TO EQUILIBRIUM IS REACHED!
...
to network_dynamics
  if relational_mobility = true [
    ...
    foreach list_agents [
      ...
      foreach list_agents [
        j ->
        if (i != j) and (matrix:get influence_m i j != 0)[           ; only run if i and j are difference agents AND they share a tie at present
            let i_opinion matrix:from-row-list (list (matrix:get-row outgoing_info_M i) )
            let j_opinion matrix:from-row-list (list (matrix:get-row outgoing_info_M j) )
            let cosim_ij cosine_sim i_opinion j_opinion
            ifelse (cosim_ij >= 0) [                                    ; when agents' opinions align
              ifelse variable_w_updating = true [                       ; set slope of sigmoid to be either
              let slope [weight_slope] of an-agent i                    ; the agent's own weight_slope
              matrix:set influence_m i j precision ( sigmoid slope .5 cosim_ij ) 5     ; if which case, ties strengthened when agents' opinions align via the sigmoid function, rounded to 5 d.p.
            ] [                                                         ; OR
              let slope sigmoid_s                                       ; the value of the global var sigmoid_s
              matrix:set influence_m i j precision ( sigmoid slope .5 cosim_ij ) 5    ; again, ties strengthened when agents' opinons align via the sigmoid function, rounded to 5 d.p.
            ]
            let weight_ij matrix:get influence_m i j               ; translate weights in influence_m to 'influences' links between agents
            ask an-agent i [
              ask out-influence-to an-agent j [
                set weight weight_ij                                          ; increase weight of influenced tie to them
                set history_interactions lput 0 history_interactions          ; add zero to history interactions, indicating opinions aligned
                set history_interactions remove-item 0 history_interactions   ; get rid of most distant memory in history of interactions (first item in list)
              ]
            ]
          ] [  ; when cosim_ij < 0
            ifelse tolerance? = false [                                ; if the agents have no tolerance, ties are always severed if they disagree
              matrix:set influence_m i j 0
              ask an-agent i [
                ask out-influence-to an-agent j [die]                  ; which means the link dies

                ; GOOD STARTING POINT FOR MAKING NEW TIES IN THIS SITUATION WOULD BE TO RECIPROCATE
              ]
            ] [  ; if tolerance? = true
              ask an-agent i [                                         ; if agents have tolerance, whether or not the tie is cut depends on how many times agents' have disagreed lately
                ask out-influence-to an-agent j [                      ; evaluation is based on history_interactions, which first has to be updated
                  set history_interactions lput 1 history_interactions              ; add 1 to history interactions, indicating opinions oposed
                  set history_interactions remove-item 0 history_interactions       ; get rid of most distant memory in history of interactions (first item in list)
                ]
                if ( ( sum [history_interactions] of influence i j ) / ( length [history_interactions] of influence i j ) ) > tolerance_threshold [
                  ask out-influence-to an-agent j [ die ]
                  matrix:set influence_m i j 0

                  ; REMINDER TO ADD NEW TIE FORMATION AS ABOVE
                ]
              ]
            ]
          ]
  ...
  let components_now length nw:weak-component-clusters       ; cound number of components
  if components_now != components [
    calc_cosim_components                                         ; if changed, compute cosine sim between two randomly selected agents' opinions (one from each)
  ]
  set components components_now

end


to standardise_influence                         ; standardise weights in influence_m so they sum to one
  foreach range n-agents [
    x ->
...
to-report sigmoid [slope shift x]                             ; sigmoid function
  ; report ( 1 / (1 + exp (slope * ( - x ))) )           ; function used in paper
  report ( 1 / (1 + exp (- slope * ( x - shift))) )         ; output can take on values from 0 - 1, lower slopes = more equal weighting of opinions that differ from own (but that don't contradict)
end


to visualise_ties_weighted                                              ; network visualisation is updated
  layout-spring agents influences  .2 6 25
  ...
  ask influences [
    set thickness ( weight / weight_most_influential ) * .1
    ; set color scale-color grey ( weight / weight_most_influential ) -2 2
  ]
end


to calc_cosim_components
  let comps nw:weak-component-clusters      ; item position in list varies, so need to store
  let i [who] of one-of item 0 comps
  let j [who] of one-of item 1 comps
  let i_opinion matrix:from-row-list (list (matrix:get-row outgoing_info_M i) )
  let j_opinion matrix:from-row-list (list (matrix:get-row outgoing_info_M j) )
  set cosim_components cosine_sim  i_opinion j_opinion
...
213
237
1 = ideological filter and ideological ego-involvement \n2 = ideological filter and non-ideological ego-involvement\n3 = unbiased filter and ideological ego-involvement\n4 = unbiased filter and non-ideological ego-involvement
7
0.0
1
...
218
289
(off in 1st simulations reported in the paper)
9
0.0
1
...
162
559
if starting_network = \"Erdős–Rényi\", then set the average degree 
11
0.0
1
...
420
88
6. Do agents have any tolerance?
11
0.0
1
...
403
139
if off, agents cut ties as soon as they disagree (default)
9
0.0
1
...
416
573
5. How is the sigmoid function set up? 
11
0.0
1
...
534
av clustering (local)
mean [ nw:clustering-coefficient ] of agents
5
1
11
...
@#$#@#$#@
## New Things I'm Working On (the pluses)

### Tolerance 

In the previous version of the model, agents' cut ties as soon as their opinions oppose a given social partner's. The present version allows for this assumption to be relaxed, so agents only cut ties if they have clashed in more than a set proportion of a given number of previous ties (both set on the interface). Until this threshold is reached, the agents continue to be influenced by the agents with oposing views. 

Though effective when agents weight the opinons of others with cosine similarities ranging from 0 - 1 more similarily (i.e., have low slopes for the sigmoid function), including tolerance can have a counterintuitive impact when agents' have more biased interaction weights (e.g., who have a strong association between cosine sim and influence weight), as well as have the same threshold and memory of previous interactions. That is,  when the population fails to consensualise, the clusters that form once are more densely connected than those that typically arrise when tolerance is not included in the model. 


### Interaction Bias

Adding individual differences in the extent to which agents display similarity driven interaction biases. 

At present, links are modeled as weighted influence ties. This does not allow for the possibility that agents are socially connected (interact) but disregard some peoples points of view. One thing to do would be to differentiate the presence/absence of ties, from their weight. 

### Realistic Starting Point for Networks 

In the previous model and at present, agents start off in a fully connected network but with different influence weights (determined by status comparisons). This represents an abstraction away from the real world where people have a limited number of social relationships and only interact with a subset of them in a given time period. 

### Tie Formation Strategies 

In the previous model and at present, agents cut ties (set influence to = 0) with others whose opinions oppose their own but do not create new ties to replace them. This means that the degree of the network decreases as time goes on. 
...
*Please note, this is a working, first-pass model. I still need to clean up the code and complete the documentation. Please don't distribute beyond the course until then.* 
...
The model could easily be extended to include heterogeous populations of agents, in terms of their cognitive style and/or motivations when updating network ties. Another option would be to start with a more realistic network structure (i.e., not assume that all agents have a degee of n-agents at t0). The severity of the network dynamic process could also be relaxed so that agents' have some degree of tolerance before they decide to cut ties to others with different opinions. 
...
<experiments>
  <experiment name="experiment" repetitions="1" runMetricsEveryStep="true">
    ...
    <go>go</go>
    <timeLimit steps="10"/>
    <metric>list (matrix:to-row-list influence_m)</metric>
    <enumeratedValueSet variable="agent_type">
      <value value="2"/>
    </enumeratedValueSet>
  </experiment>
</experiments>
